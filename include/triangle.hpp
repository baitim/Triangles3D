...
namespace triangle {
    ...
    bool is_on_one_side(const plane_t<T>& pl, const triangle_t<T>& t) {
        ...
        if (is_real_gt(pl_norm_dot_a, T{0}) &&
            is_real_gt(pl_norm_dot_b, T{0}) &&
            is_real_gt(pl_norm_dot_c, T{0}))
            return true;

        if (is_real_lt(pl_norm_dot_a, T{0}) &&
            is_real_lt(pl_norm_dot_b, T{0}) &&
            is_real_lt(pl_norm_dot_c, T{0}))
            return true;
